...
public class LoginController extends BaseController{
	...
	public AjaxJson checkuser(TSUser user, HttpServletRequest req) {
		...
		if (StringUtils.isEmpty(randCode)) {
			...
			session.setAttribute("randCode", "error_randCode");
			j.setSuccess(false);
			return j;
			...
			session.setAttribute("randCode", "error_randCode");
			return j;
		} else if (randCode.equalsIgnoreCase("error_randCode") || randCode.equalsIgnoreCase("pass_randCode")) {
			j.setMsg(mutiLangService.getLang("common.verifycode.error"));
			j.setSuccess(false);
			session.setAttribute("randCode", "error_randCode");
			return j;
		} else {
			session.setAttribute("randCode", "pass_randCode");
		}
		// IP是否被列入黑名单
		...
		// 如果通过登录名找不到用户，则使用邮箱验证
		// 删除逻辑 by eth 2018.4.28
//		if (u == null) {
...
//		}
	 ...
	 * @return
	 */
	private AjaxJson casCheckuser(String remoteUserName, HttpServletRequest req) {
		AjaxJson j = new AjaxJson();
		
		// 设置语言
//		req.getSession().setAttribute("lang", BrowserUtils.getBrowserLanguage(req));
		...
		//用户登录验证逻辑
		TSUser u = userService.checkUserExits(remoteUserName);
		...
		if (u != null && u.getStatus() != 0) {
			...
			// 获取当前用户的组织机构
			Map<String, Object> userOrgMap = systemService.findOneForJdbc("select org_id as orgId from t_s_user_org where user_id=?", u.getId());
			req.getSession().setAttribute("casvalid", "1");
			saveLoginSuccessInfo(req, u, (String) userOrgMap.get("orgId"));
		} else {
	...
	public String login(ModelMap modelMap,HttpServletRequest request,HttpServletResponse response) {
		...
		if(StringUtils.isNotEmpty(remoteUserName) && (user == null || !StringUtils.equals(remoteUserName, user.getUserName()))) {
			AjaxJson result = casCheckuser(remoteUserName, request);
			if(!result.isSuccess()) {
				request.setAttribute("casLoginErrorMsg", result.getMsg());
				return "login/cas_login_error";
			}
...
//				request.setAttribute(SSOConfig.SSO_COOKIE_MAXAGE, 60);
	...
	@RequestMapping(params = "ajaxlogout")
	@ResponseBody
	public AjaxJson ajaxlogout(HttpServletRequest request) {
		userLogout(request);
