...
public class ResourceUtil {
	 ...
	 * 12350token值
	 */
	public static ApiToken token12350 = new ApiToken();

	/**
	 * 属性文件[resources/sysConfig.properties] #默认开启模糊查询方式 1为开启 条件无需带*就能模糊查询[暂时取消]
	 * fuzzySearch=0
	 */
...
//		String requestPath = request.getRequestURI() + "?" + request.getQueryString();
		...
		if (requestPath.indexOf("&") > -1) {// 去掉其他参数(保留一个参数)
											// 例如：loginController.do?login
...
//				|| key.equals(DataBaseConstant.UPDATE_NAME)
		...
		if (key.equals(DataBaseConstant.SYS_COMPANY_CODE) || key.equals(DataBaseConstant.SYS_COMPANY_CODE_TABLE)) {
			...
			returnValue = getSessionUser().getCurrentDepart().getOrgCode().substring(0,
					Integer.valueOf(getOrgCodeLengthType()) + 1);
